...
| `process --json` stdout payload keeps stable keys (`run_id`, `pipeline_id`, `status`, `counts`, `input_dir`, `output_dir`, `farm_root`, `workspace_root`, `codex_execution_context`, `codex_home_path`, `codex_model`, `codex_reasoning_effort`, `output_schema_path`, `worker_exit_codes`, `exit_code`). | `tests/test_cli_integration_contracts.py` (`test_process_json_stdout_contract_and_workspace_root`) |
...
| Recipe-style scratch pipelines pass a scratch `--cd` under `<data_dir>/execution_contexts/` and the resolved `CODEX_HOME` into the fake `codex` subprocess. | `tests/test_fake_codex_pipeline_pack_demo.py` (`test_process_recipe_style_pipeline_uses_scratch_cd_and_profile_codex_home`) |
