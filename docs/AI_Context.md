...
Run-based determinism adds a fourth seam:

4. Frozen run assets under `<data_dir>/run_assets/<run_id>/` with `runs.config_json.frozen_assets` manifest pointer.
...
- New run-based flows also store `frozen_assets` metadata so workers can execute from frozen prompt/schema/effective-pipeline files.
- Incremental planning stores `runs.execution_fingerprint` and can reuse unchanged successful outputs into the new run output tree.
...
- Real Codex starts increment `execution_attempts` right before `run_codex_exec(...)`.
- Worker heartbeats refresh `lease_until` while execution is active and persist `last_heartbeat_at`.
- Adaptive 429 handling is run-scoped:
  - rate-limited tasks are requeued (not terminalized),
  - `tasks.rate_limit_count` increments,
  - `run_throttle_state` enforces cooldown and temporary concurrency reduction.
- Retry budget uses `effective_execution_attempts = execution_attempts - rate_limit_count`.
- Non-rate-limit failures requeue until `effective_execution_attempts >= max_attempts`, then task is marked `error`.
...
- `run create --json` and `process --json` include additive `incremental` summaries.
- `run tasks --json` includes reuse provenance fields (`reused`, `reused_from_run_id`, `reused_from_task_id`).
- `run tasks --json` and `run errors --json` now expose both counters: `attempts`/`lease_claims` and `execution_attempts`, plus `last_heartbeat_at`.
...
- Frozen run assets: `<data_dir>/run_assets/<run_id>/`
...
- Cross-cutting hidden rules: chunk README "non-obvious rules" and "merged discoveries" sections, plus caller contracts in `docs/08-external-program-reference/`
...
## Merged Deep-Dive Reference

_This section is merged from `docs/how-codex-farm-works-for-AI.md`._
...
- `attempts` (aka `lease_claims`) increases on claim, not on completion.
- `execution_attempts` increases only when Codex execution starts.
- retry budget uses `execution_attempts - rate_limit_count`, so lease churn alone does not burn retry budget.
...
3. if `effective_execution_attempts_before = execution_attempts - rate_limit_count` is already at/over budget: mark task error immediately
...
9. build:
   ...
   - staged output path `run.output_dir/.codex-farm-stage/<task_id>/<lease_token><ext>`
10. resolve Codex `--cd`:
...
13. start lease heartbeat thread (separate SQLite connection)
14. increment `execution_attempts` for this owner lease
15. run Codex wrapper against staged output path
14. if wrapper says failure -> raise runtime error
16. local schema validate staged output
17. owner-check promotion to canonical output path + `mark_task_done`
...
1. delete staged output file if present
2. trim error text to <= 1800 chars
3. if `effective_execution_attempts >= max_attempts`: mark `error`, set worker exit_code=1
4. else: `requeue_task`
...
- lifecycle control split: operator `control_state` (`active|paused|cancel_requested|canceled`) is distinct from effective run `status`
- stale-worker safety: task finalization writes are lease-token guarded so lost ownership cannot overwrite task state
