...
def test_render_prompt_template_replaces_input_text(tmp_path: Path) -> None:
    template = tmp_path / "template.txt"
    input_file = tmp_path / "recipe.json"

    template.write_text("Path={{INPUT_PATH}}\nPayload={{INPUT_TEXT}}\n", encoding="utf-8")
    input_file.write_text('{"title":"Salsa"}\n', encoding="utf-8")
...
def test_load_pipelines_reads_explicit_prompt_input_mode(tmp_path: Path) -> None:
    for folder in ("pipelines", "prompts", "schemas"):
        (tmp_path / folder).mkdir(parents=True, exist_ok=True)

    payload = {
        "pipeline_id": "demo.inline.input.v1",
        "description": "demo",
        "prompt_template_path": "prompts/demo.txt",
        "output_schema_path": "schemas/demo.schema.json",
        "prompt_input_mode": "inline",
    }
    (tmp_path / "pipelines" / "demo.inline.input.v1.json").write_text(
        json.dumps(payload, indent=2) + "\n",
        encoding="utf-8",
    )
    (tmp_path / "prompts" / "demo.txt").write_text("INPUT={{INPUT_TEXT}}\n", encoding="utf-8")
    (tmp_path / "schemas" / "demo.schema.json").write_text(
        json.dumps(
            {
                "$schema": "https://json-schema.org/draft/2020-12/schema",
    ...
    loaded = load_pipelines(tmp_path / "pipelines")
    assert loaded["demo.inline.input.v1"].prompt_input_mode == "inline"


def test_load_pipelines_rejects_unknown_prompt_input_mode(tmp_path: Path) -> None:
    for folder in ("pipelines", "prompts", "schemas"):
        (tmp_path / folder).mkdir(parents=True, exist_ok=True)

    payload = {
        "pipeline_id": "demo.bad.input.mode.v1",
        "description": "demo",
        "prompt_template_path": "prompts/demo.txt",
        "output_schema_path": "schemas/demo.schema.json",
        "prompt_input_mode": "filesystem",
    }
    (tmp_path / "pipelines" / "demo.bad.input.mode.v1.json").write_text(
        json.dumps(payload, indent=2) + "\n",
        encoding="utf-8",
    )
    (tmp_path / "prompts" / "demo.txt").write_text("INPUT={{INPUT_PATH}}\n", encoding="utf-8")
